...
if (useFuzzyLogic) {
  ...
  // OUTPUT SETS
  // durations
  var none = logic.newSet("None", 0.01, 0);
  var planck = logic.newSet("Planck", 0.2, 0);
  ...
  // OUTPUT VALUE
  var overlapDuration = logic.newValue(0.1);
  
  // weight
  logic.IF( weight.IS( tiny ) );
  logic.THEN( overlapDuration, none );
  logic.IF( weight.IS( small ) );
  logic.THEN( overlapDuration, planck );
  logic.IF( weight.IS( big ) );
  logic.THEN( overlapDuration, short );
  logic.IF( weight.IS( huge ) );
  logic.THEN( overlapDuration, mediumD );
  logic.IF( weight.IS( einstein ) );
  logic.THEN( overlapDuration, long );
  
  // strength
  logic.IF( strength.IS( big ));
  logic.THEN( overlapDuration, planck );
  logic.IF( strength.IS( huge ));
  logic.THEN( overlapDuration, none );
  ...
  logic.THEN( overlapDuration, long );
  logic.IF( friction.IS( einstein ) );
  logic.THEN( overlapDuration, veryLong );

  result = overlapDuration.crispify();
}
result;