...
For example, if a layer is constrained to two other layers with both weights at `50 %`, it stays in the center of the segment joining the two other layers. That also works with three layers and `33.3 %`, to attach the layer to the barycenter (center of mass) of the layers. Of course you can adjust the weights to change the "*mass*" of the master layers.
