...
//début du groupe d'annulation
app.beginUndoGroup("Camera Relief");
...
convergence.name = "Convergence Cameras";
...
var effet  = app.project.activeItem.selectedLayers[0].selectedProperties.pop();
		...
		//  début de groupe d'annulation
		app.beginUndoGroup("Créer un Morph");

	
var morpher = calque.Effects.addProperty("ADBE Slider Control");
morpher.name = "Morph" + effet.name;

effet.expression = "valueAtTime((effect(\"" + morpher.name + "\")(1)-thisComp.displayStartTime/thisComp.frameDuration)*thisComp.frameDuration)";


//fin du groupe d'annulation
app.endUndoGroup();
}else{alert("On ne peut pas créer d'expression sur cet effet","Moprh impossible");}
}else{alert("Veuillez sélectionner un effet où appliquer le morph","Pas d'effet sélectionné");}
}else{alert("Veuillez sélectionner un effet où appliquer le morph","Pas de calque sélectionné");}
}
...
var calques = app.project.activeItem.selectedLayers
			...
			//  début de groupe d'annulation
			app.beginUndoGroup("Duik - Lentille");

			
			
			//sortir le premier calque, le centre, et ajouter les contrôleurs
			centre = calques.shift();
			nomcentre = centre.name;
			controleurintensite = centre.Effects.addProperty("ADBE Slider Control");
			controleurintensite.name  ="Intensite";
			controleurintensite(1).setValue(100);
			controleurtaille = centre.Effects.addProperty("ADBE Slider Control");
			controleurtaille.name  ="Taille";
...
"}\n\n" +
"n=effect(\"Distance au centre\")(1);\n\n" +
"X = thisComp.width - positionAbs(calqueCentre)[0];\n" +
"Y = thisComp.height - positionAbs(calqueCentre)[1];\n\n" +
"if ( n<100 ) {\n\n" +
"i=n/100;\n" +
"j=1-i;\n\n" +
"(  [X,Y]*(i/j) + positionAbs(calqueCentre) )*j\n\n" +
"}\n\n" +
"else {[X,Y] }";

		//l'expression d'opacité
		var opaciteexpression = "n=thisComp.layer(\"" + nomcentre  + "\").effect(\"Intensite\")(1);\n" + 
			"value*n/100";
			
		//l'expression d'échelle
		var tailleexpression = "n=thisComp.layer(\"" + nomcentre  + "\").effect(\"Taille\")(1);\n" + 
			"value*n/100";
		...
		centre.transform.scale.expression = tailleexpression;
			...
			controleurposition.name = "Distance au centre";
			...
			app.endUndoGroup();
				...
				alert("Veuillez sélectionner tous les calques servant à l'effet de lentille, en commençant par le centre");
				}
...
//===========================================
	...
	palette.bounds = [300,300,560,665];
		palette.add ("statictext", [5,   5,105, 25],  "DuDuF IK Tools v7");
		var boutonhelp = palette.add ("button",[110,5,125,20],"?");
		boutonhelp.onClick = help;

		var panocre = palette.add("panel",[5,30,250,150],"Expressions");
		//bouton pour créer l'IK
		var boutonik = panocre.add("button",[5,10,115,30], "Créer un IK");
		boutonik.onClick = ik;
		//bouton pour choisir le nombre de bones
		panocre.add ("statictext",[135,15,165,30],"Bones :");
		var boutonbones = panocre.add("dropdownlist",[170,10,230,30],["1","2","2+1"]);
		...
		var boutongoal = panocre.add("button",[5,35,115,55],"IK Goal");
		...
		//bouton wiggle
		var boutonwiggle = panocre.add("button",[120,35,230,55],"Wiggle");
		...
		//bouton roue
		var boutonroue = panocre.add("button",[5,60,115,80],"Roue");
		boutonroue.onClick = creroue;
		boutonroue.helpTip = "Automatise la rotation d'une roue lors du déplacement du calque";
		//bouton morpher
		var boutonmorph = panocre.add("button",[120,60,230,80],"Morpher");
		boutonmorph.onClick = morpher;
		boutonmorph.helpTip = "Crée un morpher sur l'effet sélectionné";
		//bouton lentille
		var boutonlentille = panocre.add("button",[5,85,115,105],"Lentille");
		boutonlentille.onClick = lentille;
		boutonlentille.helpTip = "Crée une lumière parasite avec les calques sélectionnés";
		
		var panoobj = palette.add("panel",[5,155,250,200],"Objets");
		...
		var boutonbone = panoobj.add("button",[120,10,230,30],"Bone");
		boutonbone.onClick = bone;
		boutonbone.helpTip = "Crée un bone sur un coin de marionnette";
		
		var panocam = palette.add("panel",[5,205,250,250],"Cameras");
		...
		var boutontcamrelief = panocam.add("button",[120,10,230,30],"Camera Relief");
		boutontcamrelief.onClick = camrelief;
		boutontcamrelief.helpTip = "Crée des contrôleurs pour une caméra relief";
