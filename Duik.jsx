...
function fnDuIK(wnd)
{
	...
	//=================================
	{
		...
		{		
			function InstallWarning(thisObj)
			{
				...
				var versionBox = palette.add("statictext",undefined,"Duik v" + version);
				versionBox.alignment = ["center","top"];
				var warningBox = palette.add("statictext",undefined,"temp",{multiline:true});
				...
				var prefButton = palette.add("button",undefined,"Open General Preferences");
				prefButton.onClick = function ()
				{
					dialog_preferences_general();
					if (app.preferences.getPrefAsLong("Main Pref Section","Pref_SCRIPTING_FILE_NETWORK_SECURITY") == 1)
					{
						warningBox.text = "\n\nNow, you'll finally have to restart After Effects in order to use Duik.\n\nThank you for using Duik,\nmay the mighty god(s?) of animation be with you.";
						prefButton.hide();
					}
				}
			...
			var laPalette = InstallWarning(wnd);
	...
	if (Duik.usePresets)
	{
		function InstallWarning(thisObj)
		{
			...
			versionBox.alignment = ["center","top"];
			var warningBox = palette.add("statictext",undefined,"temp",{multiline:true});
			warningBox.text = "---------------- WARNING --------------\nAfter Effects must be restarted\r\nto complete the installation of Duik\n--------------------------------------------\n\nIf this message persists after you restarted After Effects, you may have to manually install the pseudo effects needed by Duik.\n\n• Open the file 'presetEffects.xml'\nin the installation folder of After Effects\n• Copy the content of the box below,\nhit CTRL+A / Cmd+A\nto select all the text,\nthen Ctrl+C / Cmd+C to copy it.";
			warningBox.minimumSize = [150,60];
			warningBox.alignment = ["center","top"];
			var xmlPresetsBox = palette.add("edittext",undefined,"test",{multiline:true});
			xmlPresetsBox.text = Duik.presetEffects;
			var warningBox2 = palette.add("statictext",undefined,"",{multiline:true});
			warningBox2.text = "• Paste this text in the file 'presetEffects.xml'\njust BEFORE the last line: '</effects>'.";
			warningBox2.minimumSize = [150,40];
			warningBox2.alignment = ["center","bottom"];
			// On définit le layout et on redessine la fenètre quand elle est resizée
			palette.layout.layout(true);
			palette.layout.resize();
			palette.onResizing = palette.onResize = function () {this.layout.resize();}
